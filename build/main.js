...
class AnalyticsMariadb extends utils.Adapter {
    ...
    async createDatapointsTotalSingle(idChannel, item) {
        ...
        try {
            if (await this.foreignObjectExists(item.idSource)) {
                ...
                await objectHandler.createOrUpdateState(this, utils, `${idChannel}.${this, this.idOld}`, 'old cumulative total value', totalState.val, sourceObj?.common, item, false, true);
                if (item.enable) {
